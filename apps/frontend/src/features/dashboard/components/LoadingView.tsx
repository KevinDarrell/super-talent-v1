...
// Step data for initial analysis mode
const steps = [
...
interface LoadingViewProps {
   ...
   * If provided, renders MODE A (Custom Overlay).
   * If omitted, renders MODE B (Step-based Analysis).
   */
  message?: string;
}

/**
 * LoadingView - Two-mode loading component
 * 
 * MODE A: Custom Overlay (used for Customize/Improve actions)
 * MODE B: Initial Analysis (step-based animation for first-time CV processing)
 */
export function LoadingView({ message }: LoadingViewProps) {
  const [currentStep, setCurrentStep] = useState(0);

  // Only run step timer in MODE B
  useEffect(() => {
    if (message) return; // Skip for custom message mode
  ...
  // ============================================
  // MODE A: CUSTOM OVERLAY (Customize/Improve actions)
  // ============================================
  if (message) {
    return (
      <div className="fixed inset-0 bg-slate-950/95 z-[100] flex flex-col items-center justify-center backdrop-blur-sm">
        ...
        <motion.h3
          ...
          className="text-xl font-serif font-bold text-champagne-400 animate-pulse"
        >
          {message}
        </motion.h3>

        <p className="text-slate-400 text-sm mt-3">This usually takes about 5-10 seconds</p>
      </div>
    );
  }

  // ============================================
  // MODE B: INITIAL ANALYSIS (Step-based animation)
  // ============================================
  return (
